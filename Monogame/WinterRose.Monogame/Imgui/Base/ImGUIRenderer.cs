...
using ImGuiNET;
using WinterRose.Monogame.Imgui.Exceptions;
using WinterRose.Monogame.Imgui.Data;
using WinterRose.Monogame.Imgui.Utilities;
using System;
...
public class ImGuiRenderer
{
    ...
    public virtual IntPtr BindTexture(Texture2D texture)
    {
        ...
        var pair = _textureData.Loaded.Where(x => x.Value == texture).FirstOrDefault();

        if (pair.Value is not null)
        {
            _textureData.TextureID = _textureData.Loaded.Count - 1;
            return pair.Key;
        }
        else
        {
            Debug.Log("Texture does not have an ImGui binding, creating...");
