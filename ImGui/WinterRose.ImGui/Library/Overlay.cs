namespace WinterRose.ImGuiUI
{
    ...
    {
        private struct TexturePtr
        {
            public IntPtr Handle;
            public uint Width;
            public uint Height;

            public TexturePtr(IntPtr handle, uint width, uint height)
            {
                Handle = handle;
                Width = width;
                Height = height;
        ...
        private ConcurrentDictionary<string, TexturePtr> loadedTexturesPtrs;
        ...
        public void AddOrGetImagePointer(string filePath, bool srgb, out IntPtr handle, out uint width, out uint height, bool IndefiniteRetry = false)
        {
            int maxRetry = 25;
        Retry:
            ...
            try
            {
                ...
                else
                {
                    ...
                    nint h = handle = this.renderer.CreateImageTexture(image, srgb ? Format.R8G8B8A8_UNorm_SRgb : Format.R8G8B8A8_UNorm);
                    uint w = width = (uint)image.Width;
                    uint he = height = (uint)image.Height;
                    loadedTexturesPtrs.AddOrUpdate(filePath, new TexturePtr(handle, width, height), (name, oldVal) => oldVal);
                }
            }
            // retry only if the error is due to the file being used by another process.
            catch (Exception e) when (IndefiniteRetry && e is IOException)
            {
                if (maxRetry-- > 0)
                {
                    Thread.Sleep(100);
        ...
        Retry:
            try
            {
                ...
                else
                {
                    ...
                    loadedTexturesPtrs.AddOrUpdate(name, new TexturePtr(handle, width, height), (name, oldVal) => oldVal);
                }
            }
            catch (Exception)
            {
        ...
        {
            if (loadedTexturesPtrs.TryRemove(key, out var data))
            {
