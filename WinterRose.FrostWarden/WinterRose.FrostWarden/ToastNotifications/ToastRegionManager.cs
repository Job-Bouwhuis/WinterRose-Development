...
public abstract class ToastRegionManager
{
    ...
    protected internal virtual void RecalculatePositions(params Toast[] except)
    {
        ...
        // --- TOP stack ---
        float topCursor = Toasts.TOAST_SPACING;
        for (int i = 0; i < activeToasts.Count; i++)
        {
            ...
            float accumulatedY = Toasts.TOAST_SPACING;
            if (activeToasts.Count >= i + 1)
            {
                for (int j = i + 1; j < activeToasts.Count; j++)
                {
                    ...
                    if (previous.StackSide == ToastStackSide.Top && !previous.IsClosing)
                        accumulatedY += previous.Height + Toasts.TOAST_SPACING;
                }
            }

            if (accumulatedY is not Toasts.TOAST_SPACING)
                topCursor += toast.Height + Toasts.TOAST_SPACING;
        ...
        for (int i = activeToasts.Count - 1; i >= 0; i--)
        {
            ...
            bottomCursor -= toast.Height + Toasts.TOAST_SPACING;
    ...
    private void PositionToast(Toast toast, ToastStackSide side)
    {
        ...
        {
            ToastStackSide.Top => Toasts.TOAST_SPACING,
            ToastStackSide.Bottom => Application.Current.Window.Height - toast.Height - Toasts.TOAST_SPACING,
    ...
    protected internal virtual void Update()
    {
        ...
        for (int i = 0; i < queueCount; i++)
        {
            ...
            float topOccupied = activeToasts
                ...
                .Sum(t => t.Height + Toasts.TOAST_SPACING);
        ...
        for (int i = 0; i < activeToasts.Count; i++)
        {
            ...
            float tNormalized = Math.Clamp(toast.AnimationElapsed / toast.AnimationDuration, 0f, 1f);
