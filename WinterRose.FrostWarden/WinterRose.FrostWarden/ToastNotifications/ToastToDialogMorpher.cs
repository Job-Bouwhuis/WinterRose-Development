...
internal static class ToastToDialogMorpher
{
    ...
    public static void Update()
    {
        for (int i = activeMorphs.Count - 1; i >= 0; i--)
        {
            ...
            ctx.Toast.Style.Background = LerpColor(ctx.Toast.Style.Background, ctx.Dialog.Style.DialogBackground, t);
            ctx.Toast.Style.Border = LerpColor(ctx.Toast.Style.Border, ctx.Dialog.Style.DialogBorder, t);
    ...
    private static void DrawMorph(MorphContext ctx)
    {
        ...
        // Background
        Color back = ctx.Toast.Style.Background.WithAlpha(ctx.Dialog.Style.DialogBackgroundRaw.A);
        ray.DrawRectangleRec(toastRect, back);

        // Border
        Color bord = ctx.Toast.Style.Border.WithAlpha(ctx.Dialog.Style.DialogBorderRaw.A);
        ...
        if (ctx.Dialog.Style.contentAlpha > 0f)
        {
            ...
            ctx.Dialog.RenderBox(ctx.Dialog.LastScaledBoudningBox, ctx.Dialog.Style.contentAlpha, ref padding, ref innerwidth, ref y);
        }
    }
